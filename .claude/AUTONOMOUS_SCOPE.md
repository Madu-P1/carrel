...
| 2026-05-19 | audit-gate trailing-redirection-strip-before-hash | Third auditor follow-up logged at end of T05+T06+T07 session (`.claude/logs/operator-followups.jsonl` 2026-05-19T15:24Z, "same class as commit 650ca37d's heredoc fix"). Added `TRAILING_REDIRECTION_PATTERNS` + `_strip_trailing_redirections` to `.claude/hooks/audit-gate.py`. Applied after `_strip_heredocs` in the hash computation. Conservative pattern set: strips trailing pipes to a known-safe utility list (tail, head, less, more, cat, column, sort, uniq, wc, tr, cut, fzf, jq, grep, rg, sed, awk, xargs, tee, nl, fmt, expand, unexpand, rev, paste, fold, join, comm, diff, cmp), standard stdout/stderr redirects (`2>&1`, `2>/dev/null`, `>/dev/null`, `</dev/null`), and tmp-file writes (`> /tmp/...`, `>> /tmp/...`, `2> /tmp/...`). Iterative — `... 2>&1 \| tee /tmp/log` strips the tee first then the redirect. Pipes to unknown programs (e.g., `... \| python3 process.py`) are preserved because they may change semantics. `is_major` still pattern-matches against the full (heredoc-stripped) command, so destructive verbs in the leading part are never missed. Verified by 17 strip cases + 4 cosmetic-redirect variants of `gh pr view 56` collapsing to one hash (`0c30ca0afe821a69`) + different verb (`gh pr list`) producing a different hash + 3 hook tests still green. |
