...
def run_case(draft: str, clauses: list[str]) -> list[dict]:
    ...
    out = []
    for claim, card in zip(env["claims"], result.claim_verdicts):
        cv = claim.get("contract_verdict") or {}
        out.append(
            {
                ...
                "quote_reason": bool(claim.get("quote_could_not_check_reason")),
            }
        )
    ...
    for claim in env["claims"]:
        if claim.get("untreated") and claim["text"] not in carded:
            out.append({"text": claim["text"], "verdict": "(untreated)", "disposition": None,
                        "anchor_type": None, "quote_reason": False})
    return out
...
CASES = [
    ...
    ("A1", "false_green", "gov present, fab quote OPENS on present line",
     'The agreement is governed by Delaware law and the parties stipulated that "the' + NL +
     'receiving party shall return all materials upon request."', [GOV],
     "receiving party shall return all materials upon request"),
    ("A2", "false_green", "gov present, fab quote WHOLLY on line 2",
     'The agreement is governed by Delaware law and the parties further agreed' + NL +
     '"the vendor shall maintain insurance of any amount it deems prudent."', [GOV],
     "the vendor shall maintain insurance of any amount it deems prudent"),
    ("A3", "false_green", "percent present, fab quote wrapped",
     'The royalty equals 50% of net fees and the schedule states that amounts are "paid' + NL +
     'only after written acceptance by the steering committee."', [PCT],
     "paid only after written acceptance by the steering committee"),
    ("A4", "false_green", "polarity present, fab quote wrapped",
     'The agreement grants Licensee a non-exclusive license and the parties confirmed that "the' + NL +
     'Software may be freely redistributed to any third party."', [POL],
     "Software may be freely redistributed to any third party"),
    ("A5", "false_green", "gov present, fab quote split across THREE lines",
     'The agreement is governed by Delaware law and it provides that "the' + NL +
     'receiving party shall return' + NL +
     'all materials upon request."', [GOV],
     "receiving party shall return all materials upon request"),
    ("A4b", "false_green", "clean polarity PRESENT + fab quote wrapped (forces a polarity green)",
     'The agreement grants Licensee a non-exclusive license to use the Software and notes that "the' + NL +
     'Licensee keeps one copy for its own files."', [POL],
     "Licensee keeps one copy for its own files"),
    ("A6", "false_green", "percent present + ALTERED quote with a [bracket] edit-mark straddling the wrap",
     'The royalty equals 50% of net fees and the clause provides "received in the [twelve]' + NL +
     'months following each report."', [PCT],
     "months following each report"),
    ("A7", "false_green", "percent present + quote whose words are SCATTERED (non-contiguous) in clause, wrapped",
     'The royalty equals 50% of net fees and the clause provides that fees are "received' + NL +
     'each report."', [PCT],
     "received each report"),

    # ---- Family B: clean verbatim wrapped quote must STAY verified (over-refusal hunt) ----
    ("B1", "over_refusal", "percent present, VERBATIM wrapped quote",
     'The royalty equals 50% of net fees and the clause provides that fees are "received' + NL +
     'in the twelve (12) months preceding each report."', [PCT], None),
    ("B2", "over_refusal", "percent present, verbatim wrapped quote with CRLF",
     'The royalty equals 50% of net fees and the clause provides that fees are "received' + CR + NL +
     'in the twelve (12) months preceding each report."', [PCT], None),
    ("B3", "over_refusal", "percent present, verbatim wrapped quote in CURLY quotes",
     'The royalty equals 50% of net fees and the clause provides that fees are ' + LDQ + 'received' + NL +
     'in the twelve (12) months preceding each report.' + RDQ, [PCT], None),
    ("B4", "over_refusal", "percent present, verbatim wrapped quote, leading-letter case flip",
     'The royalty equals 50% of net fees and the clause provides that "Received' + NL +
     'in the twelve (12) months preceding each report."', [PCT], None),

    # ---- Family C: proximity is not attribution (real sentence boundary) ----
    ("C1", "boundary", "gov present then PERIOD then fab wrapped quote (separate sentence)",
     'The agreement is governed by Delaware law. The parties also agreed that "the' + NL +
     'receiving party shall return all materials upon request."', [GOV],
     "receiving party shall return all materials upon request"),

    # ---- Family D: multi-clause pooling scope ----
    ("D1", "pooling", "gov present (clause A); fab quote is VERBATIM in sibling non-present clause B",
     'The agreement is governed by Delaware law and the parties agreed that "the' + NL +
     'receiving party shall return all materials upon request."',
     [GOV, 'Section 5. Upon termination, the receiving party shall return all materials upon request.'],
    ...
    ("E1", "quote_present_multi", "verbatim quote (quote-present) + second FABRICATED quote, same line",
     'The agreement requires that "The parties shall cooperate in good faith on all matters" '
     'and also that "no party shall ever be liable for consequential damages."', [NEUTRAL],
     "no party shall ever be liable for consequential damages"),
    ("E2", "quote_present_multi", "verbatim quote (quote-present) + second FABRICATED quote, WRAPPED",
     'The agreement requires that "The parties shall cooperate in good faith on all matters" '
     'and also that "no party shall ever' + NL + 'be liable for consequential damages."', [NEUTRAL],
     "no party shall ever be liable for consequential damages"),
]
...
def classify(case, cards) -> tuple[str, str]:
    ...
    verified_text = " ".join(" ".join(c["text"].split()) for c in cards if c["verdict"] == "verified")
    if fab is None:
        ...
        return "OVER_REFUSAL", "a verbatim wrapped quote was downgraded; the present should stay verified"
    ...
    # Also a false green if ANY card is verified whose own disposition is a non-quote
    # present AND the sentence carried the fabricated quote with no quote_reason set.
    for c in cards:
        if c["verdict"] == "verified" and c["disposition"] == "present" and c["anchor_type"] != "quote":
            # the present greened and the laundering pass did not attach a quote reason
            # anywhere in its logical group -> check the fab phrase is genuinely absent from
            # the verified surface. (Already covered above; this is the explicit present check.)
            pass
...
def main() -> int:
    ...
    for case in CASES:
        ...
        for c in cards:
            print(f"        - {c['verdict']:11} disp={str(c['disposition']):26} "
