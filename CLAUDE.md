...
- **Phase coach (Phase 1 complete, Phase 2 in progress):** calendar feed sync + WeekTimeGrid + stub coach landed in `169b84f`. Reads iCal feeds (Google / Apple / Outlook / Blackboard), renders the user's week, proposes study blocks where there's free time AND overdue SRS cards. Three rules ship: `free_block_overdue_srs` (v1, Phase 1), `deadline_imminent` (Phase 2, commit `940966bf`, scans calendar summaries for midterm/final/exam/quiz and anchors a study block to the first free 60-min window before the deadline), and `low_recent_review` (Phase 2, commit `b12359d2`, emits a review_block when at least 5 SRS cards haven't been touched in 7+ days AND are not currently overdue). One rule still pending: `gap_between_classes` (reason_code pre-listed in the schema CHECK, plug-point sketched in `services/planning/coach.py`). `refresh_active_suggestions` now dedupes on `(kind, start_at, reason_code)` so the v1 overdue rule and the Phase 2 low_recent_review rule keep their distinct signals visible even when both anchor a `review_block` at the same first free block. Tables via `migrations/0009_calendar_and_planning.sql`.
