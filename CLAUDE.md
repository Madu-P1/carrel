...
- Swift-side menu dispatch test coverage is informal. XCTest scaffold added 2026-05-16 (`macos-app/Tests/EinsteinDesktopTests/`, covers `UploadMimeTypes.swift`, `LocalApiToken.swift`, and `LaunchTelemetry` end to end — `format(milliseconds:)`, `markLaunch()`, and `markInteractive(route:performanceNowMilliseconds:)` via a `dup2`-based `StderrCapture` harness); follow-up tests for `MainMenuBuilder` should land in the same target.
