...
T1 cannot be enabled until each of these lands. PR-6 ships the best-of-K runtime and a **fail-closed interlock**: `benchmarks.t1_calibration.BEST_OF_K_GATE_ENFORCED` is `False`, the gate stamps it into every artifact, and `t1_gate.t1_permitted()` refuses any artifact whose flag is not `True`. So no honestly-minted artifact can enable T1 today; the equivalence is enforced by code, not just operator discipline. The enable PR flips the flag to `True` only after the first item below is done.

- **Mechanical best-of-K equivalence (flips `BEST_OF_K_GATE_ENFORCED`).** Either (a) the gate performs the best-of-K reduction itself over per-sentence candidate lists under `threshold_epsilon` (corpus schema becomes per-sentence with a clause list, predictions carry per-candidate scores), so the gate and runtime share one reduction; or (b) the predictions file carries a `{strategy, k}` provenance header and a B-check fails the gate unless `k == thresholds.rank_cutoff` and `strategy == "best_of_k"`. Option (a) is stronger and preferred. PR-6 deliberately does NOT build either: both commit to a predictions/corpus schema that does not exist yet, and the interlock keeps the tier dark until the corpus makes that schema concrete.
...
- ~~`predictions_sha256` recorded in `gate-pass.json`~~ **(done in PR-6).** `write_gate_pass` now pins the predictions file that produced the certified FAR alongside the other input hashes; the runtime does not re-check it (it is not a runtime input) but it anchors the strategy check above and makes the FAR reproducible.
