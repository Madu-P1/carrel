...
The fact-ledger primitive (``cachet_verify/detectors/fact_ledger.py``) decides when one
document binds the same quoted defined term to two irreconcilable figures, and
``cachet_verify/detectors/cross_document.py`` lifts that exact-term ledger across named
...
Deterministic: same inputs, same output list, always.
    ...
    from cachet_verify.detectors.crossdoc_ledger import detect_crossdoc_contradictions
