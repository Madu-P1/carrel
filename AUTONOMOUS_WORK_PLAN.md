...
**Status:** done — Tier 1 + Tier 2 shipped. Tier 1: 2026-05-27 ~01:55 GMT+2 by operator (inside PR #88 squash `3cf8739c`). Deliverables: (1) `.claude/hooks/audit-gate.py` adds `AUDIT_TIMEOUT_SECONDS=300` with auto-REJECT path; (2) all 5 gate-role agents (`independent-auditor`, `quality-rater`, `proponent`, `adversary`, `synthesizer`) have MANDATORY-write-output sections; (3) `.claude/logs/wedge-postmortems.jsonl` initialized with the 2026-05-26 wedge backfilled; auditor rule 5 + work-plan convention note added. New tests: `tests/test_routine_hooks.py::AuditGateTimeoutTests` (2 cases, both pass). Full hooks suite green at 45 tests. Tier 2: PR #89 (commit `5181732d` + doc-sync `c328601b`), rated 100/100 SHIP on 2026-05-27 by autonomous loop. Phase 4 ships `tests/test_routine_gate_smoke.py` + wires it into `script/autonomous-watchdog.sh` at startup (CARREL_SKIP_SMOKE=1 override); refuses launch on failure. Phase 5 ships orphan-claude CWD sweep in the poller's kill block + extends `tests/test_watchdog_kill.sh` with the matching test case. Incidental: REPO_ROOT now uses `pwd -P` so the CWD comparison matches `lsof` output under symlinked roots. CLAUDE.md "Running the autonomous routine" section documents the new behavior. Tier 3 (Phases 6-7) explicitly out-of-scope per operator 2026-05-27.
