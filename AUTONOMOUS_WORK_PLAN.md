...
**Status:** in_progress (reopen) — branch `feat/t08-reopen-non-vacuous-compare-2026-05-20`. First-pass parked via PR #61 (squash commit `84bd080d`). T57 (PR #63, squash commit `fa20c2c0`) wired the dispatcher + harness id-space dispatch + invocation pattern. Reopen runs the side-by-side comparison per the CLAUDE.md §Benchmarks+budgets pattern (BOTH `RETRIEVAL_USE_NODES=true` AND `INGEST_USE_DOCLING=true` on the nodes-path run; chunks-path run keeps default env). Updates `evals/reports/compare-nodes-2026-05-19.md` (or commits a new dated report) with real divergence numbers.
**Deps:** T07, T57
