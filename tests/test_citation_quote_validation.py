...
1. ``services/tutor.py:normalize_match_text`` now NFKC-normalizes
...
2. ``services/tutor.py:fuzzy_quote_match`` raises the similarity
...
class NfkcCompareTimeNormalizationTests(unittest.TestCase):
    """`normalize_match_text` must NFKC-normalize so ligature-bearing
    chunk content matches ASCII LLM quotes."""

    def test_fi_ligature_normalizes_to_ascii(self) -> None:
        result = normalize_match_text("ﬁnance")  # ﬁnance
    ...
    def test_fl_ligature_normalizes_to_ascii(self) -> None:
        result = normalize_match_text("reﬂection")  # reﬂection
    ...
    def test_validator_finds_ascii_quote_in_ligature_chunk(self) -> None:
        ...
        match = validated_citation_quote(llm_quote, chunk)
...
class FuzzyQuoteThresholdTests(unittest.TestCase):
    ...
    def test_loose_70_percent_match_now_rejected(self) -> None:
        ...
        match = fuzzy_quote_match(
            llm_quote,
            chunk,
            normalize_match_text(llm_quote),
            normalize_match_text(chunk),
        )
        self.assertIsNone(
            match,
