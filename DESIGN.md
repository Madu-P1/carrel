...
| 2026-06-17 | Cachet shell + verify surface re-toned from warm paper to the dark "Examination"; operator lifted the 2026-05-29 whole-app-rebrand deferral | With the validation funnel opening and the verify surface being the demo's eyeline, the operator lifted the "whole-app rebrand deferred until T66" hold. The Cachet shell (`cachet.module.css` `.app` local palette: `--paper`/`--paper-2`/`--sheet`/`--ink-*`/`--flag`) and the scoped verify mode (`.verifyScope` in `VerifyView.module.css`) flip from warm paper to a dark warm-ink ground, matched to the landing v2. The 2026-05-29 contract is otherwise unchanged and re-proven: monochrome surfaces; a single proofreader-oxblood accent (lightened to `#cf5b6b` to read on dark) reserved for flags only; no green (`--color-success`/`--state-ok` stay ink); the action button inverts to a paper fill with ink text so it never wears the flag oxblood; near-zero motion (the `verifyScope.test.ts` keyframe/animation guard still holds); WCAG AA re-met by the new values, the test reads the tokens dynamically and tertiary `#a4977f` clears 4.5:1 on every surface (~5.8:1 on the lightest). Token-only change through the existing scope seams; no component or verdict logic touched, so the honesty layer and the 145 contract-verify tests are unaffected. The Carrel study app (global dark study tokens) is untouched. Visual eye-pass still pending on `serve-cachet` (the Cachet shell needs the backend to render; dev mode shows Carrel and gates on the backend). |
| 2026-07-02 | Liquid Glass admitted to the Cachet shell's FLOATING CHROME only (operator directive) | Glass lives on what floats above the document (command palette, overlay scrims, the examination drawer, toasts via the shared primitives) as warm-ink translucency + backdrop blur; it never touches a reading or verdict surface (the sheet, the exhibit, the record panel stay opaque ink — glass is chrome, trust is paper). Every glass surface carries an @supports fallback to its former opaque body. The shell's flat `--glass-*` rebinds became real warm glass tokens with the highlight family rebound to paper tones, so shared glass primitives refract the Examination desk rather than Carrel's cool study theme. The verify-scope no-keyframe motion guard is untouched (glass is filters + transitions). |
