...
| 2026-06-02 | Fraunces added as the Shelf's warm ceremonial voice (Cachet PR6c), operator-approved deviation from the single-display-serif story | The verify surface keeps Instrument Serif as its only display face and the cold record keeps its Charter/Georgia reading serif (2026-05-29); neither changes. The Shelf is a different room. It is the warm register, the act of returning to a sealed record, so it earns its own voice: Fraunces, used only on the `/shelf` surface for the shelf title and the sealed/unsealed row labels, never on the verify or reading surfaces. This is a deliberate second serif, approved as a scoped exception rather than a break of the one-display-face rule, because the Shelf's job (warmth, ceremony, the human's own act) is distinct from the verify surface's job (cold proof) and reads as a different place rather than a second hero. Fraunces is self-hosted (latin + latin-ext woff2, OFL, under `frontend/src/assets/fonts/`, `@font-face` in `tokens.css` with `font-display: swap`), so the local-first guarantees hold: zero network at boot, works offline, no third-party CDN, GDPR-clean, same as Instrument Serif. The warmth stays inside its lane: the Shelf uses a warm cream ground (the chambers palette, distinct from the cold verify paper) and an ink seal disc echoing the certification seal, but it carries no oxblood (reserved for verify flags) and no verdict count (warmth never touches a verdict). The Shelf groups by the human's act (sealed / unsealed), not by engine findings. |
| 2026-06-03 | Libre Caslon Display replaces Instrument Serif as the cold display face for the Cachet shell, operator-approved | The operator found Instrument Serif "too plain and very AI-esque" (thin high-contrast display serifs have become an AI-generation tell). Libre Caslon Display (OFL, self-hosted woff2 under `frontend/src/assets/fonts/`, `@font-face` in `tokens.css`, `font-display: swap`) is the new cold display face, chosen 2026-06-02 from a three-candidate visual gate (vs Newsreader and Cardo) for its literal legal heritage. Scoped to the standalone Cachet shell via a `--font-serif` override on the `CachetApp` root (`src/cachet/cachet.module.css`), which cascades into the shared verify/shelf components inside Cachet; the Carrel app keeps Instrument Serif. Body reading serif stays Charter and Fraunces stays reserved for the warm Shelf (2026-06-02); warmth never touches a verdict. Supersedes the "Instrument Serif stays display-only" note in the 2026-05-29/05-31 rows for the Cachet product surface. |
| 2026-06-03 | SM-V3 "The Catch" is the second permitted motion moment on the verify surface (the drawn oxblood strike), operator-approved | The 2026-05-29 deviation mandated near-zero motion on `/verify`; the certification seal (2026-05-31) was the first sanctioned exception. SM-V3 is the second and final one (cap held at two; no third without retiring one): when a deterministic flag lands, the oxblood underline draws across the dead claim left to right, like a proofreader's pen ("Cachet strikes what it cannot stand behind"). The rule is a background gradient with `box-decoration-break: clone` so it follows every wrapped line, drawn by animating `background-size` 0 to 100% via WAAPI (no CSS keyframe, so the `verifyScope.test.ts` motion guard still holds). The CSS resting state is the full-width rule and a `finish()` safety net guarantees a fabricated-cite strike is never left invisible if the animation clock stalls; `prefers-reduced-motion` skips the draw and shows the static strike. Approved as a scoped exception, not a sixth signature moment, so the SM-1..SM-5 cap is untouched. Implemented in `WorkspaceMargin.tsx` (`ClaimMark`) plus `VerifyView.module.css` (`.markFlag`). Rationale in `docs/notes/2026-06-03-cachet-ux-goal-rubric.md` and `docs/notes/2026-06-03-cachet-ux-dopamine-trust-research.md`. |
